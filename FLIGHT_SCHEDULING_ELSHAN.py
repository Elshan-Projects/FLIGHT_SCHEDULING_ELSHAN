...
def schedule(n_a,n_c,time_in_days,cities):
    ...
    for  j in range(n_c):
        for jj in range(n_a):
            records.append( (j, jj, stage+jj*0.25, stage+jj*0.25+ds[j], stage+jj*0.25+2*ds[j]) )
            if(records[-1][-1]>=th):
                break
        stage=records[-1][-1]
    ...
    for m in range(len(records)):
        table.append(( cl[ records[m][0] ], "Plane_"+str( records[m][1] ), "Leaves at "+time_converter( records[m][2] ), 
                     "Arrives at "+time_converter( records[m][3] ) , "Returns at "+time_converter( records[m][4] ) ))
